					...
					<div class="panel-body">
						...
						<div class="modal-body">
							<div class="tab-content">
								...
								$proposal_id = isset($manufacturing_order) ? $manufacturing_order->proposal_id : ''; //PO Module
								...
								<div class="row">
									<div class="row">
									    <div class="col-md-4"> <!-- PO Module -->
											<label for="proposal_id" class="control-label"><?php echo _l("PO Number"); ?></label>
											<select id="proposal_id" name="proposal_id" class="selectpicker form-control" data-live-search="true" <?= $disabled_edit ? 'disabled' : null; ?>>
											    <option value=""></option>
												<?php foreach ($proposals as $proposal): ?>
													<option value="<?php echo $proposal['id']; ?>" data-iid="<?php echo $proposal['iids']; ?>"
														<?php if ($proposal_id == $proposal['id']) echo 'selected'; ?>>
														<?php echo get_proposal_po_format($proposal['id']); ?>
													</option>
												<?php endforeach; ?>
											</select>
										</div>										
									...
									<div class="row">
										...
										<div class="col-md-6">
											<?php 
											$location_id = isset($manufacturing_order) ? $manufacturing_order->location_id : '';
											$location_attrs = array('data-none-selected-text' => _l('dropdown_non_selected_tex'));
											if (!empty($disabled_edit)) {
												$location_attrs = array_merge($location_attrs, $disabled_edit);
											}
											echo render_select('location_id', $locations, array('warehouse_id', 'warehouse_name'), 'location', $location_id, $location_attrs); 
											?>
										</div>
									</div>
