	...
	for ($i = 0; $i < count($aColumns); $i++) {
		...
		if($aColumns[$i] == 'id') {
			...
			if (has_permission('manufacturing', '', 'edit') || is_admin()) {
				...
				$code .= ' | <a href="' . admin_url('manufacturing/copy_bill_of_material/' . $aRow['id']) . '" class="text-success" onclick="return confirm(\'Are you sure you want to copy this Bill of Material?\')">' . _l('Copy') . '</a>';
			}			
