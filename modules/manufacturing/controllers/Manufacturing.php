...
class Manufacturing extends AdminController
{
	 ...
	 * @return [type]             
	 */
	public function copy_bill_of_material($id)
	{
		// Get the original BOM
		$this->db->where('id', $id);
		$original = $this->db->get(db_prefix() . 'mrp_bill_of_materials')->row();

		if (!$original) {
			set_alert('danger', _l('BOM not found'));
			redirect(admin_url('manufacturing/bill_of_material_manage/'));
		}

		// Prepare data for insert
		$new_data = [
			'bom_code'           => $this->manufacturing_model->create_code('bom_code'),
			'product_id'         => $original->product_id,
			'product_variant_id' => $original->product_variant_id,
			'product_qty'        => $original->product_qty,
			'unit_id'            => $original->unit_id,
			'routing_id'         => $original->routing_id,
			'bom_type'           => $original->bom_type,
			'ready_to_produce'   => $original->ready_to_produce,
			'consumption'        => $original->consumption,
		];
        
		$insert_id = $this->manufacturing_model->add_bill_of_material($new_data);

        if($insert_id){
			$this->db->where('bill_of_material_id', $id);
			$details = $this->db->get(db_prefix() . 'mrp_bill_of_material_details')->result_array();
			foreach ($details as $row) {
				unset($row['id']); // remove auto-increment ID

				$row['bill_of_material_id'] = $insert_id; // assign new BOM ID
				$this->manufacturing_model->add_bill_of_material_detail($row);
			}			
		}

		if ($insert_id) {
			set_alert('success', _l('Copied Successfully'));
		} else {
			set_alert('warning', _l('problem_copying'));
		}

		redirect(admin_url('manufacturing/bill_of_material_detail_manage/'.$insert_id));
	}
	...
	{
	    if (!has_permission('manufacturing', '', 'view') && !has_permission('manufacturing', '', 'view_packing') ) {
	...
	public function add_edit_manufacturing_order($id = '')
	{
		...
		$data['products'] = $this->manufacturing_model->get_product();
		$data['proposals'] = $this->manufacturing_model->get_all_proposals(); //PO Module
		$data['units'] = $this->manufacturing_model->mrp_get_unit();
	...
	public function mo_mark_as_planned($id)
	{
		...
		ob_start();
		$this->start_and_stop_work_manually($id);
		ob_end_clean();
	 ...
	 * @return [type]     
	 */
	public function mo_mark_as_done($id, $quantity, $isFinal, $remaining_qty_produced)
	{
		if (!$this->input->is_ajax_request()) {
			show_404();
		}
		...
		$mo_mark_as_done = $this->manufacturing_model->mo_mark_as_done($id, (float)$quantity, (float)$isFinal, (float)$remaining_qty_produced);
	...
	public function assign_production_modal()
	{
		$data['manufacturing_order_id'] = $this->input->post('manufacturing_order_id');
		$this->load->view('manufacturing_orders/production/assign_production_modal', $data);
	}		
	...
	public function create_production()
	{
		...
		if (empty($details['bom_inventory_id']) || empty($details['qty_assigned'])) {
	        $response = [
				'success' => false,
				'message' => 'Failed to create production record. Please provide at least one BOM detail item.'
			];	
		...
		//Insert in tblmrp_bom_production_inventory
		$data = [
			'manufacturing_order_id' => $this->input->post('manufacturing_order_id'),
			'vendor_id'              => $this->input->post('vendor_id'),
			'product_name'           => $this->input->post('product_name'),
			'qty_assigned'           => $this->input->post('qty_assigned'),
			'qty_pending'            => $this->input->post('qty_assigned'),
			'price'                  => $this->input->post('price'),
			'deduct_price'           => $this->input->post('deduct_price'),
			'status'                 => 'pending',
			'comments'               => $this->input->post('comments') ?? '-',
			'is_inventory'           => $this->input->post('is_inventory'),
			'deadline'               => $this->input->post('deadline') ?? date("Y-m-d"),
			'created_at'             => $this->input->post('created_at') ? $this->input->post('created_at').' '.date("H:i:s") : date("Y-m-d H:i:s"),
		];
	
		// Insert data into the database
		$insert = $this->db->insert('tblmrp_bom_production_inventory', $data);	
		...
		if($last_insert_id){
			...
			if (!empty($details['bom_inventory_id']) && !empty($details['qty_assigned'])) {
				foreach ($details['bom_inventory_id'] as $index => $bom_inventory_id) {
					// Prepare detail record
					$detail_data = [
						'bom_production_inventory_id' => $last_insert_id,
						'bom_inventory_id'            => $bom_inventory_id,
						'qty_assigned'                => $details['qty_assigned'][$index]
					];
					
					// Insert into details table
					$this->db->insert('tblmrp_bom_production_inventory_details', $detail_data);


					// Deduct assigned quantity from tblmrp_bom_inventory
					$this->db->set('quantity_remaining', 'quantity_remaining - ' . (float)$detail_data['qty_assigned'], FALSE)->where('id', $bom_inventory_id)->update('tblmrp_bom_inventory');					
				}
			}	
	...
	public function receipt_production($bom_production_inventory_id = ""){
		...
		$this->db->where('bid.bom_production_inventory_id', $bom_production_inventory_id); // Fixed where condition
	...
	public function receive_production()
	{
		...
		$this->db->where('id', $bom_production_inventory_id);
		$updated = $this->db->update('tblmrp_bom_production_inventory');
		...
		if ($updated) {
			...
			// if($is_inventory){
			// 	//Insert or Update in Temp Inventory
			// 	$bom_inventory_exist = $this->db->where('bom_production_inventory_id', $bom_production_inventory_id)->get('tblmrp_bom_inventory')->num_rows();
			// 	if($bom_inventory_exist){
            //         //Update
			// 		$this->db->set('quantity_total', 'quantity_total + ' . $qty_received, FALSE);
			// 		$this->db->set('quantity_remaining', 'quantity_remaining + ' . $qty_received, FALSE);
			// 		$this->db->set('updated_at', date('Y-m-d H:i:s'));
				
			// 		$this->db->where('bom_production_inventory_id', $bom_production_inventory_id);
			// 		$updated = $this->db->update('tblmrp_bom_inventory');					
			// 	}else{
			// 		//Inser
			// 		$data = [
			...
			if($is_inventory) {
				if ($inventory_system == 'new'){
					// New logic -- indivisual inventory
					$bom_production_details = $this->db->where('bom_production_inventory_id', $bom_production_inventory_id)->get('tblmrp_bom_production_inventory_details')->result();
					foreach($bom_production_details as $bom_detail):
	
					   $product_name_raw = $this->db->where('id', $bom_detail->bom_inventory_id)->get('tblmrp_bom_inventory')->row()->product_name;
	
						//Insert or Update in Temp Inventory
						$bom_inventory_exist = $this->db
						->where('bom_production_inventory_id', $bom_production_inventory_id)
						->where('bom_production_inventory_detail_id', $bom_detail->id)
						->get('tblmrp_bom_inventory')->num_rows();
						if($bom_inventory_exist){
							//Update
							$this->db->set('quantity_total', 'quantity_total + ' . $qty_received, FALSE);
							$this->db->set('quantity_remaining', 'quantity_remaining + ' . $qty_received, FALSE);
			...
			$response = [
				'success' => true,
				'is_inventory' => (float)$is_inventory,
	...
	public function edit_production_modal()
	{
		...
		$this->load->view('manufacturing_orders/production/edit_production_modal', $data);
	}
	...
	public function update_production()
	{
		...
		$updated = $this->db->update('tblmrp_bom_production_inventory', $update);
	...
	public function delete_production()
	{
		...
		$bom_production = $this->db->where('id', $this->input->post("id"))->get('tblmrp_bom_production_inventory')->row();
        ...
        foreach($bom_production_details as $detail):
			$this->db->set('quantity_remaining', 'quantity_remaining + ' . $detail->qty_assigned, FALSE);
			$this->db->set('updated_at', date('Y-m-d H:i:s'));
			$this->db->where('id', $detail->bom_inventory_id);
			$updated = $this->db->update('tblmrp_bom_inventory');			
		endforeach;

		//Delete all the data
		$this->db->where('id', $detail->bom_production_inventory_id);
		$this->db->delete('tblmrp_bom_production_inventory');

		$this->db->where('bom_production_inventory_id', $detail->bom_production_inventory_id);
		$this->db->delete('tblmrp_bom_production_inventory_details');
	...
	public function start_and_stop_work_manually($manufacturing_order_id) {
		...
		//get work order
		$orders = $this->db->where('manufacturing_order_id', $manufacturing_order_id)->get('tblmrp_work_orders')->result();

		foreach ($orders as $order) {
			$work_order_id = $order->id; 

			$this->mo_mark_as_start_working($work_order_id, $manufacturing_order_id);
			$this->mo_mark_as_mark_done($work_order_id, $manufacturing_order_id);
		}
	...
	/*public function receive_production()
	{
		...
		$data = [
			...
			'qty_lost'     => $this->input->post('qty_lost'),
			'status'       => ($this->input->post('qty_pending') == 0) ? 'completed' : 'in_progress',
			'comments'     => $this->input->post('comments'),
		...
		// Update the database
		$this->db->where('id', $bom_production_inventory_id);
		$updated = $this->db->update('tblmrp_bom_production_inventory', $data);
		...
		if ($updated) {
			...
			$response = [
				'success' => false,
				'message' => 'Failed to receive production record.'
			];
		}
