...
class Manufacturing_model extends App_Model
{
	...
	public function add_manufacturing_order($data)
	{
		...
		$data['proposal_id'] = $data['proposal_id']; //PO Module
	...
	public function update_manufacturing_order($data, $id)
	{
		...
		//log Bom: 2 = Component deletion from the assembly
		...
		if(count($deletion_components) > 0){
			foreach ($deletion_components as $deletion_component) {
				$manufacturing_order_id = $id;
				$parent_product_id = 0;
				$product_id = $deletion_component['product_id'];
				$unit_id = $deletion_component['unit_id'];
				$change_type = 'Component_deletion_from_the_assembly';
				$change_quantity = $deletion_component['qty_to_consume'];
				$created_at = date('Y-m-d H:i:s');
				$staff_id = $user_id;
				$description = _l('bom_change_log_user_deletion_component_from_the_assembly');
				$rel_id = 0;
				$rel_type = '';
		...
		if(count($manufacturing_order_details['insert']) != 0){
			foreach ($manufacturing_order_details['insert'] as $add_component) {
				$manufacturing_order_id = $id;
				$parent_product_id = 0;
				$product_id = $add_component['product_id'];
				$unit_id = $add_component['unit_id'];
				$change_type = 'Component_additions_to_the_assembly';
				$change_quantity = $add_component['qty_to_consume'];
				$created_at = date('Y-m-d H:i:s');
				$staff_id = $user_id;
				$description = _l('bom_change_log_user_additions_component_to_the_assembly');
				$rel_id = 0;
				$rel_type = '';
		...
		if(count($manufacturing_order_details['update']) != 0){
			...
			foreach ($manufacturing_order_details['update'] as $update_component) {
				// check quantity is changed before update
				$this->db->where('id', $update_component['id']);
				$update_component_data = $this->db->get(db_prefix().'mrp_manufacturing_order_details')->row();
				if($update_component_data){
					if((float)$update_component_data->qty_to_consume != (float)$update_component['qty_to_consume']){
						$manufacturing_order_id = $id;
						$parent_product_id = 0;
						$product_id = $update_component['product_id'];
						$unit_id = $update_component['unit_id'];
						$change_type = 'Component_quantity_change_during_edit';
						$change_quantity = (float)$update_component['qty_to_consume'] - (float)$update_component_data->qty_to_consume;
						$created_at = date('Y-m-d H:i:s');
						$staff_id = $user_id;
						$description = _l('bom_change_log_user_component_quantity_change_during_edit');
						$rel_id = 0;
						$rel_type = '';
	...
	public function get_manufacturing_order($id)
	{
		$this->db->select('
			mo_details.*,
			tblitems.conversion_unit,
			tblitems.unit_conversion_value,
			tblitems.unit_conversion_type
		');
		$this->db->from(db_prefix().'mrp_manufacturing_order_details as mo_details');
		$this->db->join('tblitems', 'tblitems.id = mo_details.product_id', 'left');  // left join just in case
		$this->db->where('mo_details.manufacturing_order_id', $id);

		$manufacturing_order_details = $this->db->get()->result_array();

		foreach ($manufacturing_order_details as &$detail) {
			$unit_conversion = ['conversion_unit' => $detail['conversion_unit'], 'unit_conversion_value' => $detail['unit_conversion_value'], 'unit_conversion_type' => $detail['unit_conversion_type']];
			$detail['qty_to_consume_uc'] = get_unit_conversion_label($detail['qty_to_consume'], $unit_conversion);
			$detail['qty_reserved_uc']   = get_unit_conversion_label($detail['qty_reserved'], $unit_conversion);
			$detail['qty_done_uc']       = get_unit_conversion_label($detail['qty_done'], $unit_conversion);
		}
		unset($detail); // Good practice when using references		
	...
	{	
		...
		if($mo['manufacturing_order']){
			// Prioritize the explicitly selected location (warehouse) on the MO, fallback to legacy components_warehouse_id
			if (isset($mo['manufacturing_order']->location_id) && new_strlen($mo['manufacturing_order']->location_id) > 0) {
				$warehouse_id = $mo['manufacturing_order']->location_id;
			} else {
	...
	{	
        // var_dump($isFinal);
        // var_dump($quantity);
        // var_dump($remaining_qty_produced);
		// var_dump($status);
        // exit;		

		$status = ($isFinal == 1 || $quantity == $remaining_qty_produced) ? 'done' : 'in_progress';
	    ...
	    //update manufacturing order status to Finished
			...
			$update_mo_status = $this->update_manufacturing_order_status($id, ['status' => $status]);
			if($update_mo_status){
				$affected_rows++;
			}
	...
	public function mrp_add_good_receipt_voucher($mo_data)
    {
		...
		$good_receipt['manufacturing_order_id'] = $mo_data->id; //PO Module
		...
		$good_receipt_detail['manufacturing_order_id'] = $mo_data->id; //PO Module
	...
	{	
		...
		$mo = $this->get_manufacturing_order($id);
		if($mo['manufacturing_order']){
			// Use the MO's chosen location when returning inventory; fallback keeps backward compatibility
			if (isset($mo['manufacturing_order']->location_id) && new_strlen($mo['manufacturing_order']->location_id) > 0) {
				$warehouse_id = $mo['manufacturing_order']->location_id;
			} else {
				$warehouse_id = $mo['manufacturing_order']->components_warehouse_id;
	 ...
	 * @return array or object
	*/
	public function get_all_proposals() //PO Module
	{
		$this->db->select(db_prefix() . 'proposals.*');
		...
		foreach ($proposals as &$proposal) {
			$proposal['formatted_number'] = format_proposal_number($proposal['id']);
	
			// Get all iids from itemable
			$this->db->select('GROUP_CONCAT(iid) as iids');
			...
			$proposal['iids'] = $result && $result->iids ? $result->iids : '';
		}
