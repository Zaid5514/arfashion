      ...
      <div class="row">
       ...
       <label for="vendorid"><?php echo _l('vendor'); ?></label>
        <select name="vendorid" id="vendorid" class="selectpicker" data-live-search="true" data-width="100%" data-none-selected-text="<?php echo _l('ticket_settings_none_assigned'); ?>" >
            ...
            <option data-content="<?php echo pur_html_entity_decode($s['company']); ?>" value="<?php echo pur_html_entity_decode($s['userid']); ?>" <?php if(isset($debit_note) && $debit_note->vendorid == $s['userid']){ echo 'selected'; }else{ if(isset($ven) && $ven == $s['userid']){ echo 'selected';} } ?>><?php echo pur_html_entity_decode($s['company']); ?></option>
              <?php } ?>
        </select> 
      </div>
 <div class="row">
...
<div class="form-group select-placeholder">
  <?php $payment_note = (isset($debit_note) ? $debit_note->adminnote : ''); ?>
  <label for="note_type" class="control-label">Note Type</label>
  <select id="note_type" class="selectpicker form-control" data-width="100%" data-none-selected-text="<?php echo _l('Note Type'); ?>">
    <option value="">-- Select --</option>
    <option value="Advance Payment Note" <?php echo $payment_note == 'Advance Payment Note' ? 'selected' : ''; ?>>Advance Payment Note</option>
    <option value="Debit Note" <?php echo $payment_note == 'Debit Note' ? 'selected' : ''; ?>>Debit Note</option>
  </select>
</div>
</div>
</div>
