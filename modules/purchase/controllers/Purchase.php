...
class purchase extends AdminController
{
    ...
    public function vendors() {
        ...
        $this->purchase_model->add_auto_monthly_salary_ledger();
    ...
    public function salary_modal_form($staffid = '')
    {
        if ($this->input->post()) {
            $data = [
                ...
                'pause'         =>  $this->input->post('pause') ? 1 : 0,
            ];

            if ($staffid == '') {
                $this->purchase_model->add_salary_settings($data);
                set_alert('success', _l('added_successfully'));
            } else {
                $this->purchase_model->update_salary_settings($data, $this->input->post('id'));
                set_alert('success', _l('updated_successfully'));
            }

            $this->purchase_model->add_auto_monthly_salary_ledger();

            redirect(admin_url('purchase/vendor/'.$this->input->post('staff_id').'?group=salary'));
        }

        if ($staffid != '') {
            $data['setting'] = $this->purchase_model->get_salary_settings($staffid);
        }

        //$data['staff'] = $this->purchase_model->get_vendor();
        $data['staffid'] = $staffid;
        $this->load->view('vendors/modals/salary_modal_form', $data);
    }

    public function salary_ledger_form($id = '')
    {
        if ($this->input->post()) {

            /*START - logic for central payment*/
            if ($this->input->post('centralized') && $this->input->post('centralized') == 1) {
                $post = $this->input->post();

                // Normalize entry_date
                $post['entry_date'] = date("Y-m-d", strtotime($post['entry_date']));

                // Pass modified $post, not original input
                $this->purchase_model->add_salary_ledger($post);
                $this->purchase_model->add_auto_monthly_salary_ledger();
                return true;
            }
            /*END - logic for central payment*/
            ...
            if ($id == '') {
                $this->purchase_model->add_salary_ledger($this->input->post());
                set_alert('success', _l('added_successfully'));
            } else {
                $this->purchase_model->update_salary_ledger($this->input->post(), $id);
                set_alert('success', _l('updated_successfully'));
            }
        ...
        if ($id != '') {
            $data['ledger'] = $this->purchase_model->get_salary_ledger($id);
        }

        $data['payment_modes'] = $this->payment_modes_model->get();
        $data['staffid'] = $this->input->get('staffid');
        $this->load->view('vendors/modals/salary_ledger_form', $data);
    }

    public function delete_salary_ledger($id)
    {
        if (!is_admin()) {
            access_denied('Delete Salary Ledger');
        }

        $success = $this->purchase_model->delete_salary_ledger($id);

        if ($success) {
            set_alert('success', _l('deleted', 'Salary Ledger Entry'));
        } else {
            set_alert('warning', _l('problem_deleting', 'Salary Ledger Entry'));
        }
    ...
    public function get_salary_ledger_data($staff_id)
    {
        ...
        if ($this->input->is_ajax_request()) {
            $ledger = $this->purchase_model->get_filtered_salary_ledger($staff_id);
            ...
            foreach ($ledger as $row) {
                $result['aaData'][] = [
                    ...
                    _d($row['entry_date']),
                    $row['particular'],
                    app_format_money($row['salary_due'], ''),
                    app_format_money($row['payment_done'], ''),
                    app_format_money($row['running_balance'], '')
                ];
            }
    ...
    public function vendor($id = '')
    {
        ...
        if ($id == '') {
            ...
            $categoryIds = explode(',', $client->category);

            if (in_array('1', $categoryIds)) { //manufacturing
                $data['tab'][] = ['name' => 'production', 'icon' => '<i class="fa-solid fa-industry menu-icon"></i>'];
            }     
            
            if (in_array('2', $categoryIds)) { //purchaser

            }            

            if (in_array('3', $categoryIds)) { //employee
                $data['tab'][] = ['name' => 'salary', 'icon' => '<i class="fa-solid fa-rupee menu-icon"></i>'];
            }

            if (in_array('1', $categoryIds) || in_array('2', $categoryIds)) { //manufacturing or purchaser 
            ...
            if ($data['group'] == 'profile') {
               $data['customer_admins'] = $this->purchase_model->get_vendor_admins($id);
                ...
                $data['group']   = $this->input->get('group');
                $data['ledger']    = $this->purchase_model->get_filtered_salary_ledger($id, $data['from_date'], $data['to_date']);                
                //$data['ledger'] = $this->purchase_model->get_filtered_salary_ledger($id);
            }
    ...
    public function pur_request($id = ''){
        ...
        //$data['vendors'] = $this->purchase_model->get_vendor();
        $data['vendors'] = $this->purchase_model->get_vendor_select_dropdown();
...
/**
    ...
    public function add_invoice_payment($invoice){
         if ($this->input->post()) {
            ...
            $message = '';
            $success = $this->purchase_model->add_invoice_payment($data, $invoice, $group_id);
            if ($success) {
                $message = _l('added_successfully', _l('payment'));
            }
            set_alert('success', $message);
    ...
    public function convert_po_payment($pur_order){
        ...
        $success = $this->purchase_model->convert_po_to_pur_invoice($pur_order);
        $mess = '';
        if($success == true){
        ...
        echo json_encode([
            'mess' => $mess,
            'success' => $success,
            'redirect_url' => admin_url('warehouse/manage_goods_receipt?purchase_order_id='. $pur_order)
        ]);
    }
    ...
    public function debit_note($id = ''){
        ...
        if ($this->input->post()) {
            ...
            if ($id == '') {
                ...
                /*START - logic for central payment*/
                if ($this->input->post('centralized') && $this->input->post('centralized') == 1) {
                    unset($debit_note_data['centralized']);
...
$c_qty        = $this->input->post('c_qty') ?? '';
$c_type       = $this->input->post('c_type') ?? '';
$c_value      = $this->input->post('c_value') ?? '';
$c_unit       = $this->input->post('c_unit') ?? '';
$c_unit_price = $this->input->post('c_unit_price') ?? '';       

        echo $this->purchase_model->create_purchase_order_row_template($name, $item_name, $item_description, $quantity, $unit_name, $unit_price, $taxname, $item_code, $unit_id, $tax_rate, '', $discount, '', '', '', '', '', $item_key, false, $currency_rate, $to_currency, $c_qty, $c_type, $c_value, $c_unit, $c_unit_price);
    }
...
/**
    ...
    public function batch_payment_pur_modal() {
        // ini_set('display_errors', 1);
        // ini_set('display_startup_errors', 1);
        // error_reporting(E_ALL);            
        $this->load->model('invoices_model');
        ...
        $data['vendors'] = $this->purchase_model->get_vendor_select_dropdown();  
        $data['pur_invoices'] = $this->purchase_model->get_unpaid_pur_invoices();     
        $this->load->view('purchase/payments/batch_payment_pur_modal', $data);
    } 
	...
	public function add_batch_payment()
	{
		...
		$totalAdded = $this->purchase_model->add_batch_payment($this->input->post());
        if ($totalAdded > 0) {
            set_alert('success', _l('batch_payment_added_successfully', $totalAdded));

            if($this->app_modules->is_active('nex_reports')){

                if ($this->input->get('centralized') == 1) {
                    echo true;
                    exit;
                }

                return redirect(admin_url('purchase/invoices'));
                //return redirect(admin_url('nex_reports/cashflow?section=purchase-expense-report'));
            }
