...
{   
    ...
    public function get_vendor_select_dropdown($id = '', $where = [])
    {
        $this->db->select(implode(',', prefixed_table_fields_array(db_prefix() . 'pur_vendor')) . ',' . get_sql_select_vendor_company());
        ...
        if (is_numeric($id)) {
            ...
            // Append categories to 'company' field directly
            $vendors = $this->_append_category_to_company($vendors);

            return $vendors;
        }
    }

    private function _append_category_to_company($vendors)
    {
        if (empty($vendors)) return [];

        // Fetch all categories
        $category_rows = $this->db->get(db_prefix() . 'pur_vendor_cate')->result();
        $category_map = [];
        ...
        foreach ($vendors as &$vendor) {
            $cat_ids = array_filter(explode(',', $vendor['category']));
            $cat_names = [];

            foreach ($cat_ids as $cat_id) {
                if (isset($category_map[$cat_id])) {
                    $cat_names[] = $category_map[$cat_id];
                }
            }

            if (!empty($cat_names)) {
                $vendor['company'] .= " <span style='font-size: 9px; color: #777;'>(" . implode(', ', $cat_names) . ")</span>";
        ...
        if ($insert_id) {
            ...
            if(count($order_detail) > 0){
                foreach($order_detail as $key => $rqd){ 
                    ...
                    $dt_data['c_qty']         = $rqd['c_qty'];
                    $dt_data['c_type']        = $rqd['c_type'];
                    $dt_data['c_value']       = $rqd['c_value'];
                    $dt_data['c_unit']        = $rqd['c_unit'];
                    $dt_data['c_unit_price']  = $rqd['c_unit_price'];  
...
/**
     ...
     * @return     boolean  
     */
    public function add_invoice_payment($data, $invoice, $group_id = null){
    ...
    public function convert_po_to_pur_invoice($purorder){
        ...
        if($insert_id){
            ...
            $payment_id = 1; //$this->add_invoice_payment($data, $insert_id);
            if($payment_id){
                return $payment_id;
            }
            return false;
    ...
    */
    ...
    public function get_statement($vendor_id, $from, $to)
    {
        ...
        ' . db_prefix() . 'pur_invoice_payment.group_id as group_id,
        ' . db_prefix() . 'pur_invoice_payment.note as note,        
    ...
    public function get_item_v2($id = '')
    {
        ...
        $this->db->select(
            $rateCurrencyColumns . db_prefix() . 'items.id as itemid,
            rate,
            t1.taxrate as taxrate, t1.id as taxid, t1.name as taxname,
            t2.taxrate as taxrate_2, t2.id as taxid_2, t2.name as taxname_2,
            CONCAT(commodity_code,"_",description) as code_description,
            long_description,
            group_id,
            ' . db_prefix() . 'items_groups.name as group_name,
            unit,
            ' . db_prefix() . 'ware_unit_type.unit_name as unit_name,
            purchase_price,
            unit_id,
            guarantee,
            ' . db_prefix() . 'items.unit_conversion_type as c_type,
            ' . db_prefix() . 'items.unit_conversion_value as c_value,
            ' . db_prefix() . 'items.conversion_unit as c_unit'
        );
     ...
     * @return     string      
     */
    public function create_purchase_order_row_template($name = '', $item_name = '', $item_description = '', $quantity = '', $unit_name = '', $unit_price = '', $taxname = '',  $item_code = '', $unit_id = '', $tax_rate = '', $total_money = '', $discount = '', $discount_money = '', $total = '', $into_money = '', $tax_id = '', $tax_value = '', $item_key = '',$is_edit = false, $currency_rate = 1, $to_currency = '', $c_qty = '', $c_type = '', $c_value = '', $c_unit = '', $c_unit_price = '') {
        ...
        // Add names for new fields
        $name_c_qty = 'c_qty';
        $name_c_type = 'c_type';
        $name_c_value = 'c_value';
        $name_c_unit = 'c_unit';    
        $name_c_unit_price = 'c_unit_price';       
        ...
        if ($name == '') {
            ...
            // Add names for new fields
            $name_c_qty = $name . '[c_qty]';
            $name_c_type = $name . '[c_type]';
            $name_c_value = $name . '[c_value]';
            $name_c_unit = $name . '[c_unit]';            
            $name_c_unit_price = $name . '[c_unit_price]';            
        ...
        // NEW FIELDS
        $row .= '<td class="c_qty">' . 
            render_input($name_c_qty, '', $c_qty, 'number', ['min' => '0.0', 'step' => 'any', 'placeholder' => ''], [], 'no-margin', 'text-right') .
            render_input($name_c_type, '', $c_type, 'text', ['placeholder' => ''], [], 'no-margin', 'input-transparent text-right pur_input_none')
        . '</td>';

        $row .= '<td class="c_value">' . 
            render_input($name_c_value, '', $c_value, 'number', ['min' => '0.0', 'step' => 'any', 'placeholder' => ''], [], 'no-margin', 'text-right') . 
            render_input($name_c_unit, '', $c_unit, 'text', ['placeholder' => ''], [], 'no-margin', 'input-transparent text-right pur_input_none') 
        . '</td>';

        $row .= '<td class="c_unit_price">' . 
            render_input($name_c_unit_price, '', $c_unit_price, 'number', ['min' => '0.0', 'step' => 'any', 'placeholder' => ''], [], '', 'text-right')
        . '</td>';
    ...
    public function get_purchase_invoice_pdf_html($invoice_id){
        ...
        //Custome fileds added by nexgeno
        foreach (get_custom_fields('pur_invoice') as $field) {
            $value = get_custom_field_value($invoice_id, $field['id'], 'pur_invoice');
            if ($value == '') {
                continue;
            }
            //echo $field['name'] . ': ' . $value . '<br />';
            $html .= '<div class="col-md-12">
            <h4>'. $field['name'].':</h4><p>'. $value .'</p> 
            </div>';            
        }        
    ...
    public function get_all_vendors(){
        return $this->db
            ->select("v.userid as id, CONCAT(v.company, ' (', v.vendor_code, ')') AS name")
            ->distinct()
            ->from(db_prefix() . 'pur_vendor v')
            ->get()
            ->result();        
    }

    public function get_unpaid_pur_invoices($vendor_id = null) {
        //$this->db->select(db_prefix() . 'pur_invoices.*');
        $this->db->select(db_prefix() . "pur_invoices.*, CONCAT(v.company, ' (', v.vendor_code, ')') AS company");
        $this->db->join(db_prefix() . 'pur_vendor v', 'v.userid = ' . db_prefix() . 'pur_invoices.vendor', 'left');
        $this->db->where_in('payment_status', ['unpaid', 'partially_paid']);
        $this->db->where('total >', 0);
        if($vendor_id) {
            $this->db->where('v.userid', $vendor_id);
        }
        $this->db->order_by('number,YEAR(invoice_date)', 'desc');
        //$this->db->limit(50);
        $invoices = $this->db->get(db_prefix() . 'pur_invoices')->result();
        ...
        return array_map(function ($invoice) {
            $invoice->allowed_payment_modes = $this->payment_modes_model->get(); // show all modes
            $invoice->total_left_to_pay = $this->get_pur_invoice_total_left_to_pay($invoice->id, $invoice->total);

            return $invoice;
        }, $invoices);
    }

    private function get_pur_invoice_total_left_to_pay($invoice_id, $total)
    {
        return purinvoice_left_to_pay($invoice_id);
        // $this->db->select_sum('amount');
        // $this->db->where('pur_invoice', $invoice_id);
        // $this->db->where('approval_status', 2); // only approved payments
        // $paid = $this->db->get(db_prefix() . 'pur_invoice_payment')->row()->amount;

        // return floatval($total) - floatval($paid);
    }
    ...
    public function add_batch_payment($paymentsData)
    {
        ...
        $paymentIds = [];
        foreach ($paymentsData['pur_invoice'] as $data) {
            if (empty($data['pur_invoice']) || empty($data['amount']) || $data['amount'] == '0.00' || empty($data['date'])) { //|| empty($data['paymentmode']
                continue;
            }

            $payment = $data;
            unset($payment['tds_amount'], $payment['total_amount']);
            $paymentIds[] = $this->add_invoice_payment($payment, $payment['pur_invoice'], $group_id);

            $tds = $data;
            if((float)$tds['tds_amount'] > 0):
                $tds['amount']      = $tds['tds_amount'];
                $tds['paymentmode'] = 4;
                unset($tds['tds_amount'], $tds['total_amount']);
                $paymentIds[] = $this->add_invoice_payment($tds, $tds['pur_invoice'], $group_id);
            endif;
        }
    ...
    public function add_salary_settings($data)
    {
        $this->db->insert(db_prefix() . 'staff_salary_setting', $data);
        return $this->db->insert_id();
    }

    public function update_salary_settings($data, $id)
    {
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'staff_salary_setting', $data);
        return $this->db->affected_rows();
    }

    public function get_salary_settings($staffid = '')
    {
        if ($staffid == '') {
            return $this->db->get(db_prefix() . 'staff_salary_setting')->result_array();
        }

        return $this->db->where('staff_id', $staffid)->get(db_prefix() . 'staff_salary_setting')->row();
    }   

    public function add_salary_ledger($post)
    {
        $data_expense = [
            'vendor' => $post['staff_id'],
            'note' => $post['particular'],
            'category' => 23, // salary
            'date' => date('Y-m-d', strtotime($post['entry_date'])),
            'amount' => isset($post['credit']) ? $post['credit'] : 0,
            'currency' => 1,
            'paymentmode' => $post['payment_mode'] ?: null,
            'repeat_every_custom' => 1,
            'repeat_type_custom' => 'day',                    
        ];       

        $this->load->model('expenses_model');
        $expense_id = $this->expenses_model->add($data_expense); 

        $data = [
            'staff_id'       => $post['staff_id'],
            'entry_date'     => $post['entry_date'],
            'particular'     => $post['particular'],
            'debit'          => isset($post['debit']) ? $post['debit'] : 0,
            'credit'         => isset($post['credit']) ? $post['credit'] : 0,
            'payment_mode'   => $post['payment_mode'] ?? null,
            'reference_note' => $post['reference_note'] ?? null,
            'expense_id ' => $expense_id,
        ];        
        $this->db->insert(db_prefix() . 'staff_salary_ledger', $data);
        return $this->db->insert_id();
    }

    public function update_salary_ledger($post, $id)
    {
        ...
        $this->load->model('expenses_model');
        $expense_id = $this->expenses_model->update($data_expense, $post['expense_id']);
        ...
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'staff_salary_ledger', $data);
        return $this->db->affected_rows();
    }

    public function delete_salary_ledger($id)
    {
        $this->load->model('expenses_model');
        $this->expenses_model->delete($this->input->get('expense_id'));

        $this->db->where('id', $id);
        $this->db->delete(db_prefix() . 'staff_salary_ledger');
        return $this->db->affected_rows() > 0;
    }    

    public function get_salary_ledger($id = '')
    {
        if ($id == '') {
            return $this->db->get(db_prefix() . 'staff_salary_ledger')->result_array();
        }

        return $this->db->where('id', $id)->get(db_prefix() . 'staff_salary_ledger')->row();
    } 
    
    // public function get_filtered_salary_ledger($staff_id)
    // {
    //     $where = '';
    //     $sql = "
    //         SELECT
    //             l.id,
    //             l.staff_id,
    //             s.company AS staff_name,
    //             l.entry_date,
    //             l.particular,
    //             l.expense_id,
    //             l.debit AS salary_due,
    //             l.credit AS payment_done,
    //             SUM(l.debit - l.credit) OVER (
    //                 PARTITION BY l.staff_id 
    //                 ORDER BY l.entry_date, l.id
    //             ) AS running_balance
    //         FROM tblstaff_salary_ledger l
    //         JOIN tblpur_vendor s ON s.userid = l.staff_id
    //         WHERE l.staff_id = " . $this->db->escape($staff_id) . " $where
    ...
    public function get_filtered_salary_ledger($staff_id, $from_date = null, $to_date = null)
    {
        $from_date_sql = $to_date_sql = "";
        $from_date_val = $to_date_val = null;

        if (!empty($from_date)) {
            $from_date_val = to_sql_date($from_date);
            $from_date_sql = "AND l.entry_date >= " . $this->db->escape($from_date_val);
        }
        if (!empty($to_date)) {
            $to_date_val = to_sql_date($to_date);
            $to_date_sql = "AND l.entry_date <= " . $this->db->escape($to_date_val);
        }
        ...
        if ($from_date_val) {
            $balance_sql = "
                ...
                FROM tblstaff_salary_ledger
                WHERE staff_id = " . $this->db->escape($staff_id) . "
                AND entry_date < " . $this->db->escape($from_date_val) . "
            ";
            $balance_result = $this->db->query($balance_sql)->row();
            $opening_balance = $balance_result->opening_balance ?? 0;
        }

        // Main ledger query within date range
        $sql = "
            SELECT
                l.id,
                l.staff_id,
            ...
            FROM tblstaff_salary_ledger l
            JOIN tblpur_vendor s ON s.userid = l.staff_id
            WHERE l.staff_id = " . $this->db->escape($staff_id) . "
            $from_date_sql $to_date_sql
            ORDER BY l.entry_date ASC, l.id ASC
        ";

        $ledger = $this->db->query($sql)->result_array();

        // Compute running balance manually
        $balance = $opening_balance;
        $result_with_running = [];

        // Add opening balance row (optional but useful for ledger clarity)
        if ($from_date_val) {
            $result_with_running[] = [
                'id'            => 0,
                'staff_id'      => $staff_id,
                'staff_name'    => '',
                'entry_date'    => $from_date_val,
        ...
        foreach ($ledger as $row) {
            $balance += $row['salary_due'] - $row['payment_done'];
            $row['running_balance'] = $balance;
            $result_with_running[] = $row;
        }

        return $result_with_running;
    }

    public function add_auto_monthly_salary_ledger()
    {
        //$vendors = $this->db->get(db_prefix() . 'pur_vendor')->result();
        $this->db->where("FIND_IN_SET(3, category) >", 0);
        $vendors = $this->db->select('userid, category')->get(db_prefix() . 'pur_vendor')->result();    
        ...
        foreach ($vendors as $staff):
            ...
            // Check if already added for current month
            $this->db->where('staff_id', $staffid);
            $this->db->where('entry_date >=', $first_day);
            $this->db->where('entry_date <=', date("Y-m-t"));
            $this->db->where('debit >', 0);
            $exists = $this->db->get(db_prefix() . 'staff_salary_ledger')->row();

            if ($exists) {
                return false; // Already exists
            }

            // Fetch latest salary setting (effective before or on today)
            $this->db->where('staff_id', $staffid);
            $this->db->where('effective_from <=', date('Y-m-d'));
            $this->db->where('pause', 0);
            $this->db->order_by('effective_from', 'DESC');
            $salary = $this->db->get(db_prefix() . 'staff_salary_setting')->row();

            if (!$salary) {
                 continue; // Skip if already exists // No salary setting found for this staff
            }

            // ✅ Get the day from effective date
            $effective_day = date('d', strtotime($salary->effective_from));

            // ✅ Build entry date using current month & year, and day from effective date
            $entry_date = date('Y-m-' . $effective_day);

            // ✅ Handle edge case: if day exceeds last day of current month
            $last_day = date('t');
            if ((int)$effective_day > (int)$last_day) {
                $entry_date = date('Y-m-t');
            }        

            // Insert ledger entry
            $data = [
                'staff_id'       => $staffid,
                'entry_date'     => $entry_date, //$first_day,
                'particular'     => 'Salary for ' . date('F Y'),
                'debit'          => $salary->salary_amount,
                'credit'         => 0,
                'payment_mode'   => null,
                'reference_note' => null,
            ];
    ...
    // public function add_auto_monthly_salary_ledger($staffid)
    // {
    //     // Get the latest salary ledger entry for this staff
    //     $this->db->where('staff_id', $staffid);
    //     $this->db->where('debit >', 0);
    //     $this->db->order_by('entry_date', 'DESC');
    //     $last_entry = $this->db->get(db_prefix() . 'staff_salary_ledger')->row();

    //     // Determine the month to start from
    //     if ($last_entry) {
    //         $start_month = date('Y-m-01', strtotime('+1 month', strtotime($last_entry->entry_date)));
    //     } else {
    //         // First salary month? Start from current
    //         $start_month = $this->db->select('effective_from')->where('staff_id', $staffid)->get(db_prefix() . "staff_salary_setting")->row()->effective_from;
    //         $start_month = $start_month ?? date('Y-m-01');
    //     }

    //     $current_month = date('Y-m-01');
    //     $inserted_ids = [];

    //     while (strtotime($start_month) <= strtotime($current_month)) {

    //         // Check salary setting for this month
    //         $this->db->where('staff_id', $staffid);
    //         $this->db->where('effective_from <=', $start_month);
    //         $this->db->order_by('effective_from', 'DESC');
    //         $salary = $this->db->get(db_prefix() . 'staff_salary_setting')->row();
    ...
    //             $data = [
    //                 'staff_id'       => $staffid,
    //                 'entry_date'     => $start_month,
    //                 'particular'     => 'Salary for ' . date('F Y', strtotime($start_month)),
    //                 'debit'          => $salary->salary_amount,
    //                 'credit'         => 0,
    //                 'payment_mode'   => null,
    //                 'reference_note' => null,
    ...
    //         // Move to next month
    //         $start_month = date('Y-m-01', strtotime('+1 month', strtotime($start_month)));
    //     }

    //     return $inserted_ids; // return all inserted IDs, or empty array if none
    // }
