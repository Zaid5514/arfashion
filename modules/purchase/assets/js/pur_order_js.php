...
function pur_calculate_total(from_discount_money){
  ...
  $('.wh-tax-area').remove();
    ...
    quantity = $(this).find('td.c_qty input').val();
    if (quantity === '' || isNaN(quantity)) {
      quantity = 1;
      $(this).find('td.c_qty input').val(1);
    }
    ...
    //_amount = accounting.toFixed($(this).find('td.rate input').val() * quantity, app.options.decimal_places); //app.options.decimal_places
    _amount = accounting.toFixed($(this).find('td.c_unit_price input').val() * quantity, app.options.decimal_places);
    item_amount = _amount;
...
function pur_add_item_to_preview(id) {
  ...
  requestGetJSON('purchase/get_item_by_id/' + id+'/'+ currency_rate ).done(function (response) {
    ...
    $('.main input[name="unit_price"]').val('').prop('readonly', true); //.prop('readonly', true)

    $('.main input[name="unit_name"]').val(response.unit_name);
    $('.main input[name="unit_id"]').val(response.unit_id);
    $('.main input[name="quantity"]').val(1).prop('readonly', true); //.prop('readonly', true)

    
    $('.main input[name="c_type"]').val(response.c_type).prop('readonly', true);
    $('.main input[name="c_value"]').val(response.c_value).prop('readonly', true); //.prop('readonly', true)
    $('.main input[name="c_unit"]').val(response.c_unit).prop('readonly', true);
    $('.main input[name="c_unit_price"]').val(response.purchase_price);
    //$('.main textarea[name="description"]').val(response.c_type.toUpperCase());
    $('.main input[name="c_qty"]').val('1').trigger('change');
...
function pur_add_item_to_table(data, itemid) {
  ...
  pur_get_item_row_template('newitems[' + item_key + ']',data.item_name, data.description, data.quantity, data.unit_name, data.unit_price, data.taxname, data.item_code, data.unit_id, data.tax_rate, data.discount, itemid, currency_rate, to_currency, data.c_qty, data.c_type, data.c_value, data.c_unit, data.c_unit_price).done(function(output){
    table_row += output;
...
function pur_get_item_preview_values() {
  ...
  response.c_qty =  $('.invoice-item .main input[name="c_qty"]').val();
  response.c_type =  $('.invoice-item .main input[name="c_type"]').val();
  response.c_value =  $('.invoice-item .main input[name="c_value"]').val();
  response.c_unit =  $('.invoice-item .main input[name="c_unit"]').val();
  response.c_unit_price =  $('.invoice-item .main input[name="c_unit_price"]').val();
...
function pur_get_item_row_template(name, item_name, description, quantity, unit_name, unit_price, taxname,  item_code, unit_id, tax_rate, discount, item_key, currency_rate, to_currency, c_qty, c_type, c_value, c_unit, c_unit_price)  {
  "use strict";
...
console.log("c_qty:", c_qty);
console.log("c_type:", c_type);
console.log("c_value:", c_value);
console.log("c_unit:", c_unit);
console.log("c_unit_price:", c_unit_price);  
  ...
  var d = $.post(admin_url + 'purchase/get_purchase_order_row_template', {
    ...
    // After row is returned and appended in DOM, set readonly
    setTimeout(function () {
      $('input[name^="newitems"][name$="[c_qty]"]').attr('readonly', true);
      $('input[name^="newitems"][name$="[c_value]"]').attr('readonly', true);
      $('input[name^="newitems"][name$="[c_unit_price]"]').attr('readonly', true);
      $('input[name^="newitems"][name$="[quantity]"]').attr('readonly', true);
    }, 100);
  });
  jQuery.ajaxSetup({
    async: true
...
$(document).ready(function() {
    ...
    const conversionFactors = {
        meters: 39.3701,        // meters to inches
        yards: 36,              // yards to inches
        square_meters: 1550.0031,    // square meters to square inches
        square_yards: 1296,     // square yards to square inches
        pieces: 1              // pieces remain the same
    };
    ...
    const unitTypeMapping = {
        meters: 'INCH',
        yards: 'INCH',
        square_meters: 'SQ. INCH',
        square_yards: 'SQ. INCH',
        pieces: 'PCS'
    };

    // Function to calculate quantity and unit price
    function calculateQuantityAndPrice() {
        console.log("triggered");
        $('tr.main').each(function() {
            const $row = $(this);
            
            // Get input values
            const cQty = parseFloat($row.find('input[name="c_qty"]').val()) || 0;
            const cType = $row.find('input[name="c_type"]').val();
            const cValue = parseFloat($row.find('input[name="c_value"]').val()) || 0;
            const cUnit = $row.find('input[name="c_unit"]').val();
            const cUnitPrice = parseFloat($row.find('input[name="c_unit_price"]').val()) || 0;
            
            // Calculate quantity based on unit type
            let quantity = 0;
            
            if (cType && cUnit && conversionFactors[cUnit]) {
                if (cUnit.includes('square')) {
                    // Area calculation (square units)
                    quantity = cQty * cValue * conversionFactors[cUnit];
                } else {
                    // Length calculation (linear units)
                    quantity = cQty * cValue * conversionFactors[cUnit];
                }
            } else if (cUnit === 'pieces' || cUnit === 'PCS') {
                // Pieces calculation
                quantity = cQty * cValue;
            }
            
            // Calculate unit price
            let unitPrice = 0;
            if (quantity > 0 && cUnitPrice > 0) {
                if (cUnit.includes('square')) {
                    // Price per square unit
                    //unitPrice = (cUnitPrice * cQty) / (cValue * cQty);
                    unitPrice = cUnitPrice / quantity;
                } else {
                    // Price per linear unit or piece
                    unitPrice = (cUnitPrice * cQty) / quantity;
                }
            }
            
            // Update the row values
            $row.find('input[name="quantity"]').val(quantity.toFixed(2));
            $row.find('input[name="unit_price"]').val(unitPrice.toFixed(2));
            
            // Trigger change events to update calculations
            $row.find('input[name="quantity"]').trigger('change');
    ...
    $('body').on('change blur input', 'input[name="c_qty"], input[name="c_value"], input[name="c_unit_price"]', function() {
        calculateQuantityAndPrice();
    });

    $('body').on('change', 'input[name="c_type"], input[name="c_unit"]', function() {
        calculateQuantityAndPrice();
    });
