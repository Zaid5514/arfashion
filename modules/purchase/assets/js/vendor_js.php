...
<script defer>
function open_salary_modal(staffid = '') {
    $.ajax({
        url: admin_url + 'purchase/salary_modal_form/' + staffid,
        success: function(response) {
            $('#salary_modal_area').html(response);
            $('#salary_modal_area .modal').modal('show');
        }
    });
}

function salary_ledger_modal(id = '') {
    const staffid = '<?php echo $client->userid; ?>';
    $.ajax({
        url: admin_url + 'purchase/salary_ledger_form/' + id + '?staffid=' + staffid,
        success: function(response) {
            $('#salary_modal_area').html(response);
            $('#salary_modal_area .modal').modal('show');
        }
...
$(document).ready(function() {
    ...
    function handleDateRangeSelection() {
        ...
        if (rangeValue === 'period') {
            ...
            if (rangeValue) {
                try {
                    var dates = JSON.parse(rangeValue);
                    if (dates.length === 2) {
                        $('input[name="from_date"]').val(dates[0]);
                        $('input[name="to_date"]').val(dates[1]);
                    }
                } catch(e) {
    ...
    $('#range').change(function() {
        ...
        // Auto-submit the form when a predefined range is selected (not "period")
        if ($(this).val() !== 'period') {
            $('#vendor_statement_form').submit();
        }
    });
