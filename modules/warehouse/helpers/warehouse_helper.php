...
if (!function_exists('get_unit_conversion_label')) {
    function get_unit_conversion_label($quantity, $unit_conversion)
    {
        if (!isset($unit_conversion['conversion_unit']) || !isset($unit_conversion['unit_conversion_type'])) {
            return 'Invalid';
        }

        $unit = strtolower($unit_conversion['conversion_unit']);
        $type = ucfirst($unit_conversion['unit_conversion_type']); // Roll, Packet, etc.

        // Set conversion multiplier
        switch ($unit) {
            case 'meters':
                // 1 inch = 0.0254 meters
                $converted_qty = $quantity * 0.0254;
                break;

            case 'yards':
                // 1 inch = 0.0277778 yards
                $converted_qty = $quantity * 0.0277778;
                break;
            ...
            case 'square_meters':
                ...
                $converted_qty = $quantity * 0.00064516;
                break;

            case 'square_yards':
                // 1 square inch = 0.000771605 yd²
                $converted_qty = $quantity * 0.000771605;
                break;
        ...
        // Fallback if unit_conversion_value is invalid
        if (empty($unit_conversion['unit_conversion_value']) || $unit_conversion['unit_conversion_value'] == 0) {
            $unit_conversion['unit_conversion_value'] = 1;
        }

        //$total_units = floor($converted_qty / $unit_conversion['unit_conversion_value']);
        $total_units = ($converted_qty / $unit_conversion['unit_conversion_value']);
        $total_units = number_format($total_units, 2);
        $label = $type . ($total_units != 1 ? 's' : '');
        //$label = $type;

        return "{$total_units} {$label}";
    }
}

if (!function_exists('unit_conversion_options')) {
    function unit_conversion_options($type)
    {
        if ($type == 'type') {
            return ['roll', 'khaka', 'bundle', 'packet'];
        }

        if ($type == 'unit') {
            return ['meters', 'yards', 'pieces', 'square_meters', 'square_yards'];
        }
...
if (!function_exists('unit_conversion_mapping')) {
    function unit_conversion_mapping()
    {
        $unitMapping = [
            'roll'   => ['square_meters'],
            'bundle' => ['meters', 'yards'],
            'khaka'  => ['square_meters', 'square_yards'],
            'packet' => ['pieces'],
        ];
