  ...
  var expenseDropzone;
   ...
   function expenseSubmitHandler(form){
    ...
    $.post(admin_url + 'warehouse/get_item_longdescriptions/'+$(invoker).data('commodity_id')).done(function(response) {
      ...
      $('#commodity_list-add-edit input[name="unit_conversion_value"]').val(response.item_value.unit_conversion_value);
      $('#unit_conversion_type').val(response.item_value.unit_conversion_type).selectpicker('refresh');
      $('#conversion_unit').val(response.item_value.conversion_unit).selectpicker('refresh');

      // console.log(response.item_value);
...
$(document).ready(function () {
    var $conversionType = $('#unit_conversion_type');
    var $conversionUnit = $('#conversion_unit');

    // Disable conversion_unit initially
    $conversionUnit.prop('disabled', true);

    // If using Bootstrap Select
    if ($.fn.selectpicker) {
        $conversionType.selectpicker('refresh');
    ...
    if ($.fn.select2) {
        ...
        $conversionUnit.select2();
    }

    // Define reverse mapping from conversion type → units
    var unitMapping = <?php echo json_encode(unit_conversion_mapping()); ?>;

    // Function to update conversion_unit options
    function updateConversionUnitOptions(selectedType) {

        $('label[for="rate"]').html("Sale price (per " + selectedType + ")");

        var units = unitMapping[selectedType] || [];

        // Store current value
        var currentValue = $conversionUnit.val();

        // Clear and add new options
        $conversionUnit.empty();

        if (units.length > 0) {
            $conversionUnit.append($('<option>', {
                value: '',
                text: ''
            }));

            $.each(units, function (i, unit) {
                var formattedText = unit
            ...
            if (units.includes(currentValue)) {
                $conversionUnit.val(currentValue);
            }

            $conversionUnit.prop('disabled', false);
        } else {
            $conversionUnit.prop('disabled', true);
        }
    ...
    // Handle change on conversion_type
  $('body').on('change', '#unit_conversion_type', function () {
      updateConversionUnitOptions($(this).val());
  });

    // Trigger once if already selected
    if ($conversionType.val()) {
      alert(1);
        updateConversionUnitOptions($conversionType.val());
    }
});


$(document).ready(function () {
  function toggleConversionRequired() {
    var groupId = $('#group_id').val();

    if (groupId && groupId != '2') {
      // Required if not FINISH GOOD
      $('#conversion_type').attr('required', true);
      $('#unit_conversion_value').attr('required', true).attr('min', 1);
      $('#conversion_unit').attr('required', true);

      // For selectpicker refresh
      $('#conversion_type').selectpicker('refresh');
      $('#conversion_unit').selectpicker('refresh');
    } else {
      // Remove required if FINISH GOOD
      $('#conversion_type').removeAttr('required');
      $('#unit_conversion_value').removeAttr('required').removeAttr('min');
      $('#conversion_unit').removeAttr('required');
  ...
  // Run on page load
  toggleConversionRequired();

  // Run whenever group_id changes
  $('#group_id').on('changed.bs.select', function () {
    toggleConversionRequired();
  });
});
