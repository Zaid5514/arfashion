...
<script>
	...
	(function($) {
		...
		// Auto-select purchase order from URL parameter
		$(document).ready(function() {
			var urlParams = new URLSearchParams(window.location.search);
			var purchaseOrderId = urlParams.get('purchase_order_id');
			
			if (purchaseOrderId) {
				// Wait for selectpicker to be initialized
				setTimeout(function() {
					var $prOrderSelect = $('select[name="pr_order_id"]');
					if ($prOrderSelect.length) {
						// Set the value and refresh selectpicker
						$prOrderSelect.selectpicker('val', purchaseOrderId);
						$prOrderSelect.selectpicker('refresh');
						// Trigger change event to load purchase order data
						$prOrderSelect.trigger('change');
					}
				}, 1500);
			}
		});
