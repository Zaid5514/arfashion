...
$aColumns = [
	...
	'commodity_code',
	'description',
	'(SELECT GROUP_CONCAT(rel_id SEPARATOR ",") FROM ' . db_prefix() . 'itemable WHERE CAST(' . db_prefix() . 'itemable.iid AS UNSIGNED) = ' . db_prefix() . 'items.id AND rel_type = "proposal") as proposal_ids', //PO Module	
	'sku_code',
...
$item_have_variation = $this->ci->warehouse_model->arr_item_have_variation();
	...
	foreach ($rResult as $aRow) {
		...
		for ($i = 0; $i < count($aColumns); $i++) {
			...
			} elseif ($aColumns[$i] == '(SELECT GROUP_CONCAT(rel_id SEPARATOR ",") FROM tblitemable WHERE CAST(tblitemable.iid AS UNSIGNED) = tblitems.id AND rel_type = "proposal") as proposal_ids') { //PO Module
				// Display the proposal IDs (comma-separated)
				$_data = $aRow['proposal_ids'];
				
				// Or if you want to format them as links:
				if (!empty($aRow['proposal_ids'])) {
					$proposal_ids = explode(',', $aRow['proposal_ids']);
					$_data = '';
					foreach ($proposal_ids as $proposal_id) {
						$_data .= '<a href="' . admin_url('proposals/list_proposals/' . $proposal_id) . '" target="_blank">' . get_proposal_po_format($proposal_id) . '</a>, ';
					}
					$_data = rtrim($_data, ', ');
				} else {
					$_data = '';
						...
						foreach ($arr_warehouse_by_item[$aRow['id']] as $wh_key => $warehouse_value) {
							...
							if ($warehouse_value['warehouse_id'] != '' && $warehouse_value['warehouse_id'] != '0') {
								...
								if(isset($arr_warehouse_id[$warehouse_value['warehouse_id']])){
									...
									if(!$check_hide_warehouse){
                                        $unit_conversion = @$this->ci->db->select('conversion_unit,unit_conversion_type,unit_conversion_value')->where('id', $aRow['id'])->get(db_prefix() . 'items')->row_array();
										$str .= '<span class="label label-tag tag-id-1"><span class="tag">' . $arr_warehouse_id[$warehouse_value['warehouse_id']]['warehouse_name'] . ': ( '.$quantity_by_warehouse.' )</span><span class="hide">, </span></span>';
                                        $str .= '<div style="font-size: 12px;margin-bottom:3px;">' . get_unit_conversion_label($quantity_by_warehouse, $unit_conversion) . '</div>';
