...
class Payments_model extends App_Model
{
    ...
    public function add_batch_payment($paymentsData)
    {
        ...
        foreach ($paymentsData['invoice'] as $data) {
            if (empty($data['invoiceid']) || empty($data['amount']) || $data['amount'] == '0.00' || empty($data['date']) || empty($data['paymentmode'])) {
                continue;
            }

            /* TDS payment record - start */
            if((float)$data['tds_amount'] > 0):
                $tds_data                  = $data;
                $tds_data['group_id']          = $group_id;
                $tds_data['date']          = to_sql_date($data['date']);
                $tds_data['daterecorded']  = date('Y-m-d H:i:s');
                $tds_data['amount']        = $data['tds_amount'];
                $tds_data['paymentmode']   = 4;
                $tds_data                  = hooks()->apply_filters('before_payment_recorded', $tds_data);
                //$this->db->insert(db_prefix() . 'invoicepaymentrecords', $data);
                $this->db->insert(db_prefix() . 'invoicepaymentrecords', array_diff_key($tds_data, array_flip(['tds_amount', 'total_amount'])));
                $tds_insert_id = $this->db->insert_id();

                if ($tds_insert_id) {
                    // Make sure invoice object is loaded before using
                    if (!isset($invoice)) {
                        $invoice = $this->invoices_model->get($data['invoiceid']);
                    }                   

                    // Log invoice activity for TDS payment
                    $this->invoices_model->log_invoice_activity(
                        $data['invoiceid'],
                        'invoice_activity_payment_made_by_staff',
                        false,
                        serialize([
                            'TDS: ' . app_format_money($data['tds_amount'], $invoice->currency_name),
                            '<a href="' . admin_url('payments/payment/' . $tds_insert_id) . '" target="_blank">#' . $tds_insert_id . '</a>',
                        ])
                    );

                    // Log system activity
                    log_activity('TDS Payment Recorded [ID:' . $tds_insert_id . ', Invoice Number: ' . format_invoice_number($invoice->id) . ', TDS: ' . app_format_money(
                        $data['tds_amount'],
                        $invoice->currency_name
                    ) . ']');
                }
            endif;
            /* TDS payment record - end */            
            ...
            //$this->db->insert(db_prefix() . 'invoicepaymentrecords', $data);
            $this->db->insert(db_prefix() . 'invoicepaymentrecords', array_diff_key($data, array_flip(['tds_amount', 'total_amount'])));
            $insert_id = $this->db->insert_id();
