...
function get_proposal_tracking() { //PO Module
  if (typeof proposal_id == "undefined") {
    return;
  }
  requestGet("proposals/get_proposal_tracking/" + proposal_id).done(function (
    response
  ) {
    $("body").find("#proposal-tracking").html(response);
...
function add_item_to_preview(id) {
  requestGetJSON("invoice_items/get_item_by_id/" + id).done(function (
    ...
    $('.main input[name="iid"]').val(response.itemid); //PO Module
...
function add_item_to_table(data, itemid, merge_invoice, bill_expense) {
  ...
  get_taxes_dropdown_template(tax_name, data.taxname).done(function (
    ...
    table_row += `<input type="hidden" name="newitems[${item_key}][iid]" value="${data.itemid}">`;  //PO Module   
...
function get_item_preview_values() {
  var response = {};
  response.itemid = $('.main input[name="iid"]').val(); //PO Module
